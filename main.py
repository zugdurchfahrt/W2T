...
async def main():
    ...
    # Find the WhatsApp chat text file in the export directory
    ...
    try:
        ...
        print(f"\nAvailable dialogs ({len(active_dialogs)} total):")
        for i, dialog in enumerate(active_dialogs):
        ...
        try:
            ...
            confirm_text = getattr(check_peer, 'confirm_text', str(check_peer))
            print(f"\n{confirm_text}")
        except RPCError as e:
        ...
        # Ask for explicit confirmation before starting the import
        ...
        for idx, filename in enumerate(valid_media_files):
            ...
            print(f"Uploading media {idx+1}/{len(valid_media_files)}: {filename} ({file_size_mb:.1f} MB)...")

            try:
                uploaded_media = await client.upload_file(filepath)
            except (ConnectionError, asyncio.exceptions.IncompleteReadError, OSError) as e:
                print(f"[!] Network error uploading {filename}: {e}. Skipping this file.")
                continue
